...
def MD(V_max):
    ...
    l=len(temp)
    l2=int(l//2)
    return np.mean(pressure[l2:l]) , np.mean(temp[l2:l])
...
plt.plot(Ts, slope * np.array(Ts) + intercept, label=f'Linear fit: P = ({slope:.4f})T + ({intercept:.4f})', linestyle='--', color='r')
plt.legend()
print(f"slope: {slope:}, b: {intercept:}")
plt.plot(Ts, Ps)
plt.xlabel('temp')
plt.ylabel('P')
plt.grid(True)
